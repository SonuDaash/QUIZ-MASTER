...
import { createUserProfile, createStudentProfile, getUserProfile } from '@/lib/firebase/firestore';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Create student profile in Firestore if student
      if (role === 'student') {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setError(err.message || 'Google sign-up failed. Please check your Firebase Console configuration.');
      }
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-[#1e3a5f]">Create an account</h2>
        <p className="text-sm text-gray-500 mt-1">Join Smart Mind Quiz Platform</p>
      </div>
      ...
      >
        ...
        {googleLoading ? 'Connecting...' : `Sign up with Google as ${role === 'admin' ? 'Admin' : 'Student'}`}
      </button>
          ...
          <div>
            ...
            <div className="mt-1">
              <select
                ...
                className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-white focus:outline-none focus:ring-[#3b82f6] focus:border-[#3b82f6] sm:text-sm"
              >
      ...
      <div className="mt-6 text-center">
        <Link href="/login" className="font-medium text-[#3b82f6] hover:text-blue-500 text-sm">
